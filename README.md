...
Design Anchor 让 AI 每次写代码前先读取项目的风格方向、组件约束和布局质量原则。第 50 个页面的按钮颜色、hover 状态、间距节奏和第 1 个页面一模一样——因为它们基于同一份风格定义，不是 AI 临时编的。

**美化是最终目标，规则让美化可持续。** 每一份风格 prompt 包含完整的视觉规格：设计哲学、配色策略、字体搭配、阴影与质感、组件视觉样式、动效原则、签名元素和反模式。AI 不是在套 token 框架——而是在执行一套有品质感的视觉设计。

**关键色治理：**
- 主交互色（按钮、链接、选中态、focus 环）统一追溯到 `primary` token
- hover / focus / disabled 从 token 派生，不是每个组件自己硬编码
- 状态色（成功/警告/错误/信息）全产品一致
- 装饰性颜色、渐变、阴影、页面特有的视觉设计——保留原样，不做 token 化
...
- 检测到不达标组件时，提供替换建议并由你决定：整体替换或渐进修改

## 设计审查

每一个 UI 产出在交付前自动审查，13 项质量门槛全部通过才能标记完成：

1. **视觉完成度**——页面看起来精致、有品质感，不是线框图或黑白骨架
2. WCAG AA 对比度
3. 所有交互元素有 hover / focus 状态
4. 空状态、加载状态、错误状态都设计过
5. 常见视口宽度不破版
6. **关键色统一**——主题色、CTA、状态色等结构性颜色通过 token 对齐，装饰性颜色保留原有设计
7. **主色全产品一致**——不同页面的按钮不能蓝得不一样
8. **交互状态统一**——hover / disabled 从 token 派生，全局一致
9. 单一 icon 库
10. 所有表单字段有可见 label
11. 破坏性操作有确认门槛
12. 键盘可达
